      ...
      <div className='sm:flex-column flex flex-col md:flex-row md:gap-7'>
        <div className='w-full sm:w-full  md:w-1/2'>
          <div className='text-black w-3/4 mx-auto leading-tight text-40px text-center sm:w-full md:text-left md:w-5/6 md:mx-0 lg:text-64px  font-bold dark:text-white'>
            {t("main.free")} <span className='text-[#637381]'>{t("main.online")}</span> <br /> <span className='text-[#FA541C] underline'>{t("main.Courses")}</span> {t("main.from")}
          </div>
          ...
          <div className='flex flex-col gap-7 items-center mt-14 sm:flex-row justify-center md:w-full lg:justify-start'>
            <button className='bg-[#FA541C] text-white flex gap-2 px-6 py-3 rounded-lg md:px-3 font-bold md:text-[16px]'>{t("main.ready")} <img src={main} alt="mainRight" /></button>
            <div className='flex gap-5 items-center'>
                ...
                <div className='text-black text-18px font-semibold md:text-[16px] dark:text-white'>{t("main.watch")}</div>
            </div>
          </div>
