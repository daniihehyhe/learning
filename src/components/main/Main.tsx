      ...
      <div className='sm:flex-column flex flex-col md:flex-row md:gap-7'>
        <div className='w-full sm:w-full  md:w-1/2'>
          <div className='text-black w-3/4 mx-auto leading-tight text-40px text-center sm:w-full md:text-left md:w-5/6 md:mx-0 lg:text-64px  font-bold'>
            Free <span className='text-silver'>Online</span> <br /> <span className='text-orange underline'>Courses</span> From The Experts
          </div>
          <div className='text-descrColor text-16px mt-2 md:text-left text-center px-5 sm:mt-5 md:px-0 lg:w-[446px] font-regular'>
          ...
          <div className='flex flex-col gap-7 items-center mt-14 sm:flex-row justify-center md:w-full lg:justify-start'>
            <button className='bg-orange text-white flex gap-2 px-6 py-3 rounded-lg md:px-3 font-bold md:text-[16px]'>Ready Start <img src={main} alt="mainRight" /></button>
          ...
          <div className='flex items-center gap-9 mt-12 justify-center sm:gap-32 md:gap-16 lg:gap-36 lg:justify-start'>
            {
                mainData.map((item) => (
                    <div key={item.count}>
                        <div className='text-black text-24px font-bold relative before:absolute before:content-before before:w-[24px] before:h-[24px] before:-left-2 before:bg-[#FFC1073D] before:-z-10 before:rounded-full'>{item.count}</div>
                        <div className='text-descrColor mt-1 text-16px font-regular'>{item.title}</div>
                    </div>
                ))
            }
          </div>
